...
class TestTAASuite:
    ...
    @pytest.mark.asyncio
    async def test_case_send_and_get_taa(
            self, pool_handler, wallet_handler, get_default_trustee, taa_text, taa_ver
    ):
        trustee_did, _ = get_default_trustee
        ...
        req = await ledger.append_txn_author_agreement_acceptance_to_request(
            req, taa_text, taa_ver, None, aml_key, int(time.time()//(24*60*60)*(24*60*60))
        )
    ...
    @pytest.mark.asyncio
    async def test_aml_special_case(
    ...
    async def test_case_taa_with_xfer(
        ...
        req, _ = await payment.build_payment_req(
            wallet_handler, trustee_did, json.dumps([source1]), json.dumps(
                [{"recipient": address2, "amount": 100*100000}, {"recipient": address1, "amount": 900*100000}]
            ), extra
        )
        res1 = json.loads(await ledger.sign_and_submit_request(pool_handler, wallet_handler, trustee_did, req))
        print(res1)
        ...
        assert res3['op'] == 'REPLY'
        extra = await payment.prepare_payment_extra_with_acceptance_data(
            None, taa_text, taa_ver, None, aml_key, int(time.mktime(datetime.now().date().timetuple()))
        )
        req, _ = await payment.build_payment_req(
            wallet_handler, trustee_did, json.dumps([source1]), json.dumps(
                [{"recipient": address2, "amount": 100*100000}, {"recipient": address1, "amount": 900*100000}]
        ...
        req, _ = await payment.build_payment_req(
            wallet_handler, trustee_did, json.dumps([source2]), json.dumps(
                [{"recipient": address2, "amount": 200*100000}, {"recipient": address1, "amount": 700*100000}]
            ), None
        )
