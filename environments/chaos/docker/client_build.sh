...
docker build -t 'indycore_for_chaos' --build-arg uid=$USER_ID -f ${SCRIPT_DIR}/core.ubuntu.dockerfile $SCRIPT_DIR
PRIV_KEY_PATH=./dockerkey
docker build -t 'indycore' --build-arg ips=$IPS --build-arg priv_key=${PRIV_KEY_PATH} -f ${SCRIPT_DIR}/chaos.client.init.dockerfile $SCRIPT_DIR
