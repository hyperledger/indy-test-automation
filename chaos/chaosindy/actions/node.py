...
def clean_by_node_name(node: str,
    ...
    :param node: A node alias/hostname. Required.
    :type node: str
    ...
    if not stop_by_node_name(node, gracefully=False, force=True,
        ssh_config_file=ssh_config_file):
        logger.debug("Failed to stop indy-node service before cleaning node:" \
                     " %s", node)
        return False
    ...
    try:
        if clean_ledger:
            logger.debug("Cleaning ledger data. Removing data directory from"\
                         " >%s< on node: %s", ledger_data_dir, node)
            result = executor.execute(node, "rm -rf {}/data".format(
                ledger_data_dir), as_sudo=True)
            if result.return_code != 0:
                logger.error("Failed to delete the ledger data directory >%s<" \
                             " on node %s", rule, node)
                return False
        if clean_logs:
            logger.debug("Cleaning ledger data. Removing logs from >%s< on "\
                         "node: %s", ledger_data_dir, node)
            result = executor.execute(node, "rm -f {}/*.log* || true".format(
                ledger_data_dir), as_sudo=True)
            if result.return_code != 0:
                logger.error("Failed to delete the ledger logs >%s<" \
                             " on node %s", rule, node)
                return False
        if clean_iptables:
            logger.debug("Removing INPUT iptables rules on node: %s", node)
            result = executor.execute(node, "iptables -F INPUT".format(
                ledger_data_dir), as_sudo=True)
            if result.return_code != 0:
                logger.error("Failed to delete INPUT iptables rules on node %s",
                             node)
                return False
        ...
        # Start indy-node service
        if not start_by_node_name(node, ssh_config_file=ssh_config_file):
            logger.debug("Failed to start indy-node service after cleaning node:" \
                         " %s", node)
            return False
